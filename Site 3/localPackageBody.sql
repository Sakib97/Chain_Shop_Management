...
cursor discount_cur is 
	select * from discount where discount_date = to_char(sysdate);
...
begin 
	...
	if cur_avail >= v_amount then
		...
		total_price := v_amount * cur_price;
			...
			if discount_cur%found then
			  total_price := total_price -to_number((total_price * cur_percentage)/100);
			else DBMS_OUTPUT.PUT_LINE('No discount !');
			end if;
