create or replace package body globalPackage as 
	...
	begin
		if site_no = 1 then 
			delete employee@site2 where eid = v_eid ;
			delete employee@site3 where eid = v_eid ;
			insert into employee values(v_eid,v_ename,1);
			commit ;

		elsif site_no = 2 then 
			delete employee where eid = v_eid ;
			delete employee@site3 where eid = v_eid ;
			insert into employee@site2 values(v_eid,v_ename,2);
			commit ;
...
begin 
	...
	insert into product@site2 (pid, pname, unit_available, unit_price) values (id, name, avail, price);
	commit;
	elsif siteno = 3 then
...
function get_total_sell_of_a_date(input_date in date)
	...
	cursor sell_cur2 is
	select sum(price) from records@site2 where purchase_date = input_date;
	...
	begin
		if site_no = 1 then
			...
			loop
				...
				delete discount@site2 ;
				for i in 1 .. day_no
			loop
				insert into discount@site2 values(todays_date, parcentage);
				todays_date := to_date(todays_date) + 1;
				...
				for i in 1 .. day_no
			loop
				insert into discount@site3 values(todays_date, parcentage);
				todays_date := to_date(todays_date) + 1;
